/**
 * One-time script to generate a Google Business Profile (GBP) API refresh token.
 * Run: node scripts/get-gbp-token.mjs
 * Authorize with eitan@wao.co.il when the browser opens.
 * Paste the resulting refresh token into .env.local as GBP_REFRESH_TOKEN.
 *
 * Requires the same GCP project's OAuth client (GOOGLE_ADS_CLIENT_ID/SECRET) to have
 * "My Business Account Management API" and "My Business Business Information API"
 * enabled under APIs & Services > Library.
 */
...
const REDIRECT_PORT = 4243; // different port than gsc-get-token.mjs so both can be run independently
...
console.log('\n=== Google Business Profile OAuth2 Token Generator ===\n');
...
const server = http.createServer(async (req, res) => {
  ...
  console.log(`GBP_CLIENT_ID="${CLIENT_ID}"`);
  console.log(`GBP_CLIENT_SECRET="${CLIENT_SECRET}"`);
