...
נשאר רק דבר אחד. לחתום על החוזה, וּלשלם.
בדיוק ככה זה עכשיו עם הדומיין שלך.
במודול הראשון כבר בחרת שֵׁם.
...
במודול הראשון עברת חמישה קריטריונים, וּבחרת שֵׁם.
זה היה השלב הקשה, וּכבר מאחוריך.
היום אין עוד החלטות לקבל.
יש רק פעולה אחת. לשלם, וּלוודא שהדומיין באמת שלך.
-->
...
הדרך הכי פשוטה, וגם בעברית מלאה — **box.co.il**:

1. **מחפשים** את השם שבחרת במודול 1, ובוחרים סיומת (co.il, וגם com אם תרצה)
2. **משלמים** בקופה — על השם, בלי תוספות שלא צריך
3. **מוודאים** שהגיע מייל אישור בעלות, רשום על שמך — לא על שם מישהו אחר

> **גילוי נאות:** הקישור ל-box.co.il הוא קישור שותפים. אם תירשם דרכו, WAO עשויה לקבל עמלה.

מעדיף סיומת בינלאומית, או שכבר יש לך העדפה? גם **GoDaddy** וגם **Namecheap** יעשו את העבודה (בממשק אנגלי).

<!--
התהליך כולו מתפרק לשלושה צעדים.
הדרך הכי פשוטה היא דרך בוקס קו-אי-אל, והממשק כולו בעברית.
ראשון, תיכנס, תחפש את השֵׁם שבחרת, ותבחר סיומת.
קו-אי-אל ללָקוּחַ ישראלי, וגם קום אם תרצה.
שני, תעבור לקופה, ותשלם על השֵׁם שבחרת.
שלישי, וזה החשוב מכולם, תוודא שהגיע מייל אישור בעלות.
על שמך. לא על שֵׁם המעצב, ולא על שֵׁם חברת השיווק.
רק גילוי קטן וחשוב. בוקס היא שותפה שלנו.
אם תירשם דרכה, ייתכן שנקבל עמלה.
מעדיף סיומת בינלאומית? גם גו-דאדי וגם ניים-צ׳יפ יעשו את העבודה.
-->
...
תזכור את הסיפור משיעור אחד. לָקוּחַ שאיבד את כל האתר שלו.
...
ראשון, שֵׁם הבעלים בטופס. זה אתה, או שֵׁם העסק שלך.
...
תיכנס לחברת הרישום שבחרת, ותרכוש את השֵׁם שלך.
...
היא מתחדשת לבד כל שנה, וּמייל אישור הבעלות שמור אצלך.
אבל כתובת בלי בניין היא רק שלט ריק.
בשיעור הבא תעלה את האתר שבנית לאוויר.
תוך עשר דקות, בלי לשלם על אחסון. נתראה שָׁם.
-->
