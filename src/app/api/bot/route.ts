...
const TURN_QUESTIONS: Record<number, string> = {
  0: "יאללה, בוא נתחיל — ספר לי על התחום שלך. איך כולם קוראים לו בזירת העבודה? (לא את התואר — אלא את השם האמיתי של התחום.)",   1: "ואיך קוראים לעסק?\n(אם אין שם — השם שלך הוא המותג)\nיופי!",
  2: "שם יפה! ומה השם הפרטי שלך?",
      ...
      case 4:
        ...
        if (data.turnIndex === 4 && data.businessNiche && data.serviceModel) {
          ...
          // Creative professionals (photographer, designer, stylist)
            ...
            paceLeadResponse = "העיניים שלך זה המוצר — אף אחד לא יכול להעתיק את האינטואיציה שלך.\nאז נבנה דף שמביא יותר אנשים לראות את מה שאתה רואה.";
          }
