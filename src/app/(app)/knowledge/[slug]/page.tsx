...
//   • Number-only parens like (50%–60%) — digit-leading, no Latin or Hebrew letters inside.
//     Without this, the Unicode Bidi algorithm treats the en-dash as RTL-neutral and reverses
//     the numbers. Wrapping as a single LTR atom preserves the correct (50%–60%) order.
//   • Latin runs optionally preceded by ( [ { — all one island so "(Anchor Text)" stays correct.
//   • Comma+space separator so "(Chrome, Firefox, Edge)" becomes one island.
//   • Internal dot allowed only when followed by a letter/digit (example.com, v2.0).
//   • Standalone bracket/brace fallback for unmatched brackets before Hebrew text.
//   • () excluded from fallback so Hebrew parentheticals get natural BiDi mirroring.
function renderMixed(text: string): React.ReactNode {
  ...
  // 3. Number-only parens — digit-leading, content has no letters (Latin or Hebrew).
  //    The Unicode BiDi algorithm would mirror brackets when numbers (weak chars) default to
  //    RTL base direction, reversing 50%–60% to 60%–50%. LTR wrapping fixes both issues.
  // 4. Latin runs optionally preceded/followed by ( [ { } ] — one island.
  // 5. Standalone bracket/brace fallback — excludes () so Hebrew parentheticals
  //    are left for the BiDi algorithm to mirror naturally in RTL context.
  const re =
    /(https?:\/\/[^\s<>]*|\/[A-Za-z][A-Za-z0-9\-_.]*(?:\/[A-Za-z0-9\-_.]*)*\/?|\(\d[^)a-zA-Z֐-׿]*\)|[([{]*[A-Za-z][A-Za-z0-9\-\/='"{}]*(?:\.(?=[A-Za-z0-9])[A-Za-z0-9\-\/='"{}]*)*(?:[,]?[ ]+[A-Za-z][A-Za-z0-9\-\/='"{}]*(?:\.(?=[A-Za-z0-9])[A-Za-z0-9\-\/='"{}]*)*)*[)\]]*|[[\]{}])/g;
  let last = 0;
