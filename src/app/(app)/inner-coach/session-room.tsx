...
import IntakeApproval from './intake-approval';
import ReflectionCard, { type ReflectionPayload } from './reflection-card';
...
interface InnerCoachSessionResponse {
...
/**
 * Inner Coach voice room — mode-driven (M2'/M3'): mints against whatever mode
 * the page picked (today's automatic pick, or an explicit override), and after
 * the call ends, posts the transcript then runs the reflect step, rendering
 * either the intake draft-approval flow or the Language Mirror results.
 */
export default function InnerCoachSessionRoom({ mode, personaName, situation, timeCapMin, personaId, beliefId }: Props) {
  ...
  const postTranscriptAndReflect = useCallback(async (turns: TranscriptTurn[]) => {
    ...
    fetch('/api/inner-coach/transcript', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  const handleStop = useCallback(() => {
    ...
    postTranscriptAndReflect(transcriptRef.current);
  }, [stopTimer, pushTurn, teardownAudio, postTranscriptAndReflect]);
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/inner-coach/session', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mode }),
      });
      const data = (await res.json()) as InnerCoachSessionResponse;
      if (!res.ok) {
      ...
      const session = await ai.live.connect({
        ...
        config: {
          ...
          speechConfig: {
            languageCode: 'he-IL',
            ...(data.voice ? { voiceConfig: { prebuiltVoiceConfig: { voiceName: data.voice } } } : {}),
          },
        ...
        callbacks: {
          ...
          onerror: e => {
            console.error('[inner-coach][gemini] session error', e?.message ?? e);
            setErrorMsg('שגיאה בחיבור ל-Gemini Live — נסה שוב.');
          },
          onclose: e => console.log('[inner-coach][gemini] session closed', e?.reason ?? ''),
        },
      });
      ...
      console.error('[inner-coach][gemini] start failed', err);
      setErrorMsg(err instanceof Error ? err.message : 'לא הצלחנו להתחיל שיחה.');
      setPhase('idle');
      teardownAudio();
    }
  }, [hardCapSec, handleStop, flushPlaybackQueue, playChunk, pushTurn, teardownAudio, personaName, mode]);
