...
export function isLegacyOperationalRecord(record: StoredEpisodeAnalysis): boolean { return Boolean(record.result.failure); }
export function episodeSummary(record: StoredEpisodeAnalysis): EpisodeSummary | null { if (isLegacyOperationalRecord(record)) return null; return { episodeId: record.episodeId, createdAt: record.createdAt, currentTitle: record.input.currentTitle ?? '', recommendedTitle: record.result.titles[0]?.title ?? '', decision: record.result.decision, confidence: record.theme.confidence, theme: record.theme.theme, selectedKeyword: record.result.selectedKeyword?.phrase ?? '', searchVolume: record.result.selectedKeyword?.searchVolume ?? null }; }
